...
`
    ...
    async function callHost(method, args) {
      ...
      try {
        ...
        if (controller !== null && controller.signal.aborted) throw new Error('请求宿主超时（15 秒无响应）')
        throw err
      } finally {
    ...
    function EntryBody(props) {
      ...
      return h('div', null,
        ...
        overflowing ? h('div', { className: 'im-more', onClick: () => setOpen(!open) }, open ? '收起' : '展开全部') : null)
    }
    ...
    function InstructionPanel() {
      ...
      async function handleImportFile(file) {
        try {
          ...
          try {
            ...
            setError('导入文件不是有效的 JSON（' + String(err && err.message ? err.message : err) + '）')
            return
          }
          await send('import-data', { payload })
        } catch (err) {
          setError(String(err && err.message ? err.message : err))
        }
      }
      ...
      const header = h('div', { className: 'im-card', key: 'head' },
            ...
            h('input', {
              ...
              onBlur: () => {
                ...
                if (!Number.isFinite(parsed) || parsed < limits.minBudget || parsed > limits.maxBudget) {
                  ...
                  setError('上限需在 ' + limits.minBudget + ' ~ ' + limits.maxBudget + ' 之间')
                  return
                }
        ...
        entries.length > 0 ? h('div', { key: 'search' },
          h('input', {
            className: 'im-input',
            value: query,
            placeholder: '搜索指令：标题 / 内容 / 适用场景（共 ' + entries.length + ' 条）',
            onChange: (ev) => setQuery(ev.target.value),
          }),
          query.trim() !== '' ? h('div', { className: 'im-meta im-search-meta' }, '匹配 ' + visible.length + ' / ' + entries.length + ' 条') : null) : null,
        h('div', { className: 'im-row-wrap' },
          ...
          h('button', { className: 'im-btn', disabled: busy, onClick: doExport }, '导出'),
          h('button', {
            className: 'im-btn',
            disabled: busy,
            onClick: () => {
            ...
            h('button', {
              ...
              onClick: () => {
                ...
                if (typeof window.confirm === 'function'
                  && !window.confirm('确定删除「' + (entry.title !== '' ? entry.title : '未命名指令') + '」？删除后无法恢复。')) return
                send('delete-entry', { id: entry.id })
              },
            }, '删除')),
      ...
      const list = entries.length === 0
        ...
        : visible.length === 0
          ? h('div', { className: 'im-empty', key: 'no-match' }, '没有匹配“' + query.trim() + '”的指令。')
